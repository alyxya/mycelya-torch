...
def _remote_kernel_fallback_impl(
    ...
    if has_alias_info and is_mutable:
        log.info(
            f"🚨 Mutable operation with alias_info (NOT treated as view): {op_name}"
        )
        ...
        log.info(
            f"   - alias_info details: {[r.alias_info for r in op._schema.returns]}"
        )
        log.info(f"   - arg count: {len(args)}, kwarg keys: {list(kwargs.keys())}")
        ...
        for i, arg in enumerate(args):
            if isinstance(arg, torch.Tensor):
                tensor_args.append(
                    f"arg[{i}]: {arg.device.type}:{arg.device.index if hasattr(arg.device, 'index') else 'N/A'}, shape={arg.shape}"
                )
        if tensor_args:
            log.info(f"   - tensor args: {'; '.join(tensor_args)}")
        ...
        for key, value in kwargs.items():
            if isinstance(value, torch.Tensor):
                tensor_kwargs.append(
                    f"{key}: {value.device.type}:{value.device.index if hasattr(value.device, 'index') else 'N/A'}, shape={value.shape}"
                )
        if tensor_kwargs:
    ...
    def validate_and_convert_to_meta_tensor(
        tensor: torch.Tensor, tensor_id: int
    ) -> torch.Tensor:
    ...
    log.debug(
        f"Meta args devices: {[arg.device.type if isinstance(arg, torch.Tensor) else type(arg).__name__ for arg in meta_args]}"
    )
    log.debug(
        f"Meta kwargs devices: {[value.device.type if isinstance(value, torch.Tensor) else type(value).__name__ for value in meta_kwargs.values()]}"
    )

    try:
        log.debug(f"🔧 About to execute meta operation: {op_name}")
        log.debug(
            f"Meta args: {[type(arg).__name__ + (' on ' + arg.device.type if isinstance(arg, torch.Tensor) else '') for arg in meta_args]}"
        )
        meta_result = op(*meta_args, **meta_kwargs)
        ...
        if id(meta_output) in input_meta_tensor_ids:
            ...
            if out_tensor is not None and len(output_tensors) == i:
                ...
                with lazy_allocation_context():
                    ...
                    log.debug(
                        f"✅ torch.empty completed for output {i} with lazy allocation"
                    )

                # Apply stride if different from default
                if meta_output.stride() != new_tensor.stride():
                    log.debug(
                        f"🔧 About to call torch.as_strided for output {i}: {meta_output.shape}, {meta_output.stride()}, {meta_output.storage_offset()}"
                    )
    ...
    if out_tensor is not None and out_tensor.numel() == 0:
        ...
        if out_idx >= 0 and out_idx < len(meta_outputs):
            ...
            if meta_out.numel() > 0:
                log.debug(
                    f"Pre-operation: resizing empty 'out' tensor from {out_tensor.shape} to {meta_out.shape}"
                )
    ...
    if orchestrator is not None:
        log.debug(
            f"🔧 STEP 6: Executing {op_name} remotely with meta-based output handling"
        )
        ...
        for i, (output_tensor, meta_output) in enumerate(
            zip(output_tensors, meta_outputs)
        ):
            ...
            offset_differs = (
                output_tensor.storage_offset() != meta_output.storage_offset()
            )

            if shape_differs or stride_differs or offset_differs:
                log.debug(
                    f"Post-operation: output tensor {i} metadata differs from meta result"
                )
                log.debug(
                    f"  Tensor: shape={output_tensor.shape}, stride={output_tensor.stride()}, offset={output_tensor.storage_offset()}"
                )
                log.debug(
                    f"  Meta:   shape={meta_output.shape}, stride={meta_output.stride()}, offset={meta_output.storage_offset()}"
                )
                ...
                if (
                    output_tensor.stride() != meta_output.stride()
                    or output_tensor.storage_offset() != meta_output.storage_offset()
                ):
                    ...
                    log.debug(
                        f"  Applied stride {meta_output.stride()} and offset {meta_output.storage_offset()}"
                    )
