...
class TestRetainGradFunctionality:
    ...
    def test_retain_grad_basic(self, shared_devices):
        ...
        NumericalTestUtils.assert_tensors_close(
            z_remote.grad.cpu(), expected_z_grad,
            msg="z gradient should be ones"
        )
        NumericalTestUtils.assert_tensors_close(
            x_remote.grad.cpu(), expected_x_grad,
            msg="x gradient should equal y"
        )
        NumericalTestUtils.assert_tensors_close(
            y_remote.grad.cpu(), expected_y_grad,
            msg="y gradient should equal x"
        )
    
    def test_retain_grad_multiple_intermediates(self, shared_devices):
    ...
    def test_retain_grad_comparison_with_cpu(self, shared_devices):
        ...
        NumericalTestUtils.assert_tensors_close(
            ...
            msg="Remote x gradient should match CPU"
        )
        NumericalTestUtils.assert_tensors_close(
            y_remote.grad.cpu(), y_cpu.grad,
            msg="Remote y gradient should match CPU"
        )
        NumericalTestUtils.assert_tensors_close(
            z_remote.grad.cpu(), z_cpu.grad,
            msg="Remote z gradient should match CPU"
        )
    
    def test_without_retain_grad_no_intermediate_gradients(self, shared_devices):
