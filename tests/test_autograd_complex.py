...
class TestComplexComputationalGraphs:
    ...
    def test_multiple_operations_chain(self, shared_devices):
        ...
        y_remote = y_remote + 1
        y_remote = y_remote ** 2
        y_remote = y_remote.sum()
        y_remote.backward()
        
        # Compare gradients
        NumericalTestUtils.assert_tensors_close(
            x_remote.grad.cpu(), cpu_grad,
            msg="Complex chain gradient mismatch"
        )
    
    def test_branching_computational_graph(self, shared_devices):
        ...
        NumericalTestUtils.assert_tensors_close(
            x_remote.grad.cpu(), cpu_grad,
            msg="Branching graph gradient mismatch"
        )
    
    def test_multiple_inputs_graph(self, shared_devices):
        """Test gradients with multiple input tensors."""
        x_cpu = torch.randn(2, 2, requires_grad=True)
        y_cpu = torch.randn(2, 2, requires_grad=True)
        
        x_remote = x_cpu.clone().detach().requires_grad_(True).to(shared_devices["t4"].device())
        y_remote = y_cpu.clone().detach().requires_grad_(True).to(shared_devices["t4"].device())
        ...
        # Compare gradients
        NumericalTestUtils.assert_tensors_close(
            x_remote.grad.cpu(), x_cpu_grad,
            msg="Multi-input x gradient mismatch"
        )
        NumericalTestUtils.assert_tensors_close(
            y_remote.grad.cpu(), y_cpu_grad,
            msg="Multi-input y gradient mismatch"
        )
...
class TestAdvancedMatrixOperations:
    ...
    def test_matrix_chain_multiplication(self, shared_devices):
        ...
        A_remote = A_cpu.clone().detach().requires_grad_(True).to(shared_devices["t4"].device())
        B_remote = B_cpu.clone().detach().requires_grad_(True).to(shared_devices["t4"].device())
        C_remote = C_cpu.clone().detach().requires_grad_(True).to(shared_devices["t4"].device())
        ...
        NumericalTestUtils.assert_tensors_close(
            A_remote.grad.cpu(), A_cpu.grad,
            msg="Matrix A gradient mismatch in chain"
        )
        NumericalTestUtils.assert_tensors_close(
            B_remote.grad.cpu(), B_cpu.grad,
            msg="Matrix B gradient mismatch in chain"
        )
        NumericalTestUtils.assert_tensors_close(
            C_remote.grad.cpu(), C_cpu.grad,
            msg="Matrix C gradient mismatch in chain"
        )
    
    def test_matrix_element_wise_combination(self, shared_devices):
        ...
        # Compare gradients
        NumericalTestUtils.assert_tensors_close(
            X_remote.grad.cpu(), X_cpu.grad,
            msg="X gradient mismatch in combined operations"
        )
        NumericalTestUtils.assert_tensors_close(
            Y_remote.grad.cpu(), Y_cpu.grad,
...
class TestHigherOrderGradients:
    ...
    def test_second_order_gradients(self, shared_devices):
        ...
        try:
            ...
            NumericalTestUtils.assert_tensors_close(
                second_grad_remote.cpu(), second_grad_cpu,
...
class TestGradientWithViewOperations:
    ...
    def test_gradients_through_multiple_views(self, shared_devices):
        ...
        try:
            ...
            NumericalTestUtils.assert_tensors_close(
                x_remote.grad.cpu(), cpu_grad,
                msg="Multi-view gradient mismatch"
            )
        except (RuntimeError, NotImplementedError):
            pytest.skip("Complex view operations not supported")
    
    def test_gradients_with_permute_operations(self, shared_devices):
...
class TestNeuralNetworkGradients:
    ...
    def test_linear_layer_gradients(self, shared_devices):
        ...
        weight_remote = weight_cpu.clone().detach().requires_grad_(True).to(shared_devices["t4"].device())
        bias_remote = bias_cpu.clone().detach().requires_grad_(True).to(shared_devices["t4"].device())
        ...
        NumericalTestUtils.assert_tensors_close(
            weight_remote.grad.cpu(), weight_cpu.grad,
            msg="Weight gradient mismatch in linear layer"
        )
        NumericalTestUtils.assert_tensors_close(
            bias_remote.grad.cpu(), bias_cpu.grad,
            msg="Bias gradient mismatch in linear layer"
        )
    
    def test_activation_function_gradients(self, shared_devices):
...
class TestMultipleBackwardPasses:
    ...
    def test_multiple_backward_passes_same_graph(self, shared_devices):
        ...
        NumericalTestUtils.assert_tensors_close(
            second_grad.cpu(), expected_grad.cpu(),
            msg="Gradient accumulation failed"
        )
    
    def test_gradient_accumulation_different_operations(self, shared_devices):
...
class TestCrossDeviceGradientRestrictions:
    ...
    def test_cross_device_gradient_restriction(self, shared_devices):
        """Test that cross-device operations properly restrict gradients."""
        if len([k for k in TestConstants.DEVICE_KEYS if k in shared_devices]) < 2:
            pytest.skip("Need at least 2 devices for cross-device testing")
        
        # Get two different devices
        device_keys = [k for k in TestConstants.DEVICE_KEYS if k in shared_devices][:2]
        device1_key, device2_key = device_keys[0], device_keys[1]
        
        x_remote1 = DeviceTestUtils.create_remote_tensor(
            (2, 2), shared_devices, device1_key, requires_grad=True
        )
        y_remote2 = DeviceTestUtils.create_remote_tensor(
            (2, 2), shared_devices, device2_key, requires_grad=True
        )
...
def test_parametrized_linear_gradients(shared_devices, batch_size, input_size, output_size):
