...
def test_basic_tensor_creation_debug(shared_devices):
    ...
    print(f"Remote tensor: {x_remote.shape}, {x_remote.dtype}, device: {x_remote.device}")
...
def test_direct_tensor_creation(shared_devices):
    ...
    for name, create_fn in creation_methods:
        try:
            ...
            print(f"  Values range: [{tensor.min().item():.4f}, {tensor.max().item():.4f}]")
        except Exception as e:
...
def test_mixed_device_gradient_computation(shared_devices):
    ...
    print(f"Gradient a: {a_cpu.grad.shape}, range: [{a_cpu.grad.min():.4f}, {a_cpu.grad.max():.4f}]")
    print(f"Gradient b: {b_cpu.grad.shape}, range: [{b_cpu.grad.min():.4f}, {b_cpu.grad.max():.4f}]")
    
    # Verify gradients are reasonable (not zero, not NaN)
    assert not torch.allclose(a_cpu.grad, torch.zeros_like(a_cpu)), "Gradients should be non-zero"
    assert not torch.allclose(b_cpu.grad, torch.zeros_like(b_cpu)), "Gradients should be non-zero"
...
def test_neural_network_training_simulation(shared_devices):
    ...
    for param, name in zip(parameters, param_names):
        ...
        print(f"✓ {name} gradient: shape={param.grad.shape}, range=[{param.grad.min():.6f}, {param.grad.max():.6f}]")
