...
class TestCPUToRemoteTransfers:
    ...
    def test_cpu_to_remote_various_shapes(self, shared_devices):
        ...
        test_shapes = TestConstants.SMALL_SHAPES + TestConstants.TENSOR_3D_SHAPES + TestConstants.TENSOR_4D_SHAPES
    ...
    def test_cpu_to_remote_with_dtype_conversion(self, shared_devices):
        ...
        remote_tensor = cpu_tensor.to(shared_devices["t4"].device(), dtype=torch.float64)
...
class TestRemoteToCPUTransfers:
    ...
    def test_remote_to_cpu_after_operations(self, shared_devices):
        ...
        NumericalTestUtils.assert_tensors_close(result_back_to_cpu, result_cpu_reference)
...
class TestCrossDeviceTransferRestrictions:
    ...
    def test_cross_device_operation_restriction(self, shared_devices):
        ...
        ErrorTestUtils.assert_cross_device_fails(
            tensor1, tensor2, lambda x, y: x + y
        )
        ErrorTestUtils.assert_cross_device_fails(
            tensor1, tensor2, lambda x, y: x.mm(y)
        )
    ...
    def test_cross_device_transfer_direct(self, shared_devices):
        ...
        tensor_device1 = DeviceTestUtils.create_remote_tensor((2, 2), shared_devices, device1_key)
    ...
    def test_cross_device_via_cpu_transfer(self, shared_devices):
        ...
        DeviceTestUtils.verify_device_properties(tensor_device2, shared_devices[device2_key])
...
class TestTransferWithConversions:
    ...
    def test_transfer_with_copy_parameter(self, shared_devices):
        ...
        DeviceTestUtils.verify_device_properties(remote_tensor_copy, shared_devices["t4"])
        ...
        DeviceTestUtils.verify_device_properties(remote_tensor_no_copy, shared_devices["t4"])
    ...
    def test_transfer_with_non_blocking(self, shared_devices):
        ...
        try:
            ...
            remote_tensor = cpu_tensor.to(shared_devices["t4"].device(), non_blocking=True)
            assert remote_tensor is not None
            DeviceTestUtils.verify_device_properties(remote_tensor, shared_devices["t4"])
        except (RuntimeError, NotImplementedError):
...
@pytest.mark.parametrize("transfer_chain", [
    ["cpu", "t4", "cpu"],
    ["cpu", "t4", "cpu", "t4", "cpu"],
])
    ...
    for target_device in transfer_chain[1:]:  # Skip first 'cpu' entry
        if target_device == "cpu":
            ...
            if target_device in shared_devices:
                current_tensor = current_tensor.to(shared_devices[target_device].device())
                DeviceTestUtils.verify_device_properties(current_tensor, shared_devices[target_device])
            else:
                pytest.skip(f"Device {target_device} not available")
    
    # Final tensor should match original numerically
    final_cpu = current_tensor.cpu() if current_tensor.device.type != "cpu" else current_tensor
