...
class TestCPUToRemoteTransfers:
    ...
    def test_cpu_to_remote_various_dtypes(self, shared_machines, dtype):
...
class TestRemoteToCPUTransfers:
    ...
    def test_remote_to_cpu_various_shapes(self, shared_machines):
    ...
    def test_remote_to_cpu_after_operations(self, shared_machines):
...
class TestCrossDeviceTransferRestrictions:
    ...
    def test_cross_device_operation_restriction(self, shared_machines):
        ...
        tensor1 = DeviceTestUtils.create_remote_tensor(
            (2, 2), shared_machines, device1_key
        )
        tensor2 = DeviceTestUtils.create_remote_tensor(
            (2, 2), shared_machines, device2_key
        )
    ...
    def test_cross_device_transfer_direct(self, shared_machines):
        ...
        with pytest.raises(
            ...
            tensor_device1.to(shared_machines[device2_key].device())

    def test_cross_device_via_cpu_transfer(self, shared_machines):
        ...
        tensor_device1 = original_data.to(shared_machines[device1_key].device())

        # Transfer via CPU
        cpu_intermediate = tensor_device1.cpu()
        tensor_device2 = cpu_intermediate.to(shared_machines[device2_key].device())

        # Verify successful transfer
        DeviceTestUtils.verify_machine_properties(
            tensor_device2, shared_machines[device2_key]
        )
...
class TestTransferWithConversions:
    ...
    def test_transfer_with_copy_parameter(self, shared_machines):
    ...
    def test_transfer_dtype_and_device_combined(self, shared_machines):
...
class TestTransferErrorHandling:
    ...
    def test_invalid_device_transfer(self, shared_machines):
    ...
    def test_transfer_large_tensors(self, shared_machines):
...
class TestTransferMemoryEfficiency:
    ...
    def test_repeated_transfers_memory_cleanup(self, shared_machines):
    ...
    def test_transfer_with_gradient_memory(self, shared_machines):
        ...
        if target_device == "cpu":
            ...
            if target_device in shared_machines:
                current_tensor = current_tensor.to(
                    shared_machines[target_device].device()
                )
                DeviceTestUtils.verify_machine_properties(
                    current_tensor, shared_machines[target_device]
                )
            else:
...
def test_parametrized_device_transfers(shared_machines, machine_key):
