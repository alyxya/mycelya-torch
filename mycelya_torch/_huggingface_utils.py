...
def create_huggingface_model_from_remote(
    ...
    # Track storage IDs for the local storage registry and mapping
    created_storage_ids = []
    storage_mapping = {}  # local_storage_id -> remote_storage_id
    ...
    for name, param_metadata in state_dict_metadata.items():
        ...
        remote_storage_id = param_metadata["storage_id"]

        # Create remote tensor stub (this will generate a new local storage ID)
        remote_tensor = create_remote_tensor_stub(
            storage_id=remote_storage_id,  # Pass for reference, but won't be used
        ...
        # Extract the actual local storage ID generated by the C++ allocator
        local_storage_id = remote_tensor.untyped_storage().data_ptr()

        # Add to storage mapping: local -> remote
        storage_mapping[local_storage_id] = remote_storage_id
        ...
        created_storage_ids.append(local_storage_id)

        # Replace parameter in model
        _set_nested_parameter(model, name, nn.Parameter(remote_tensor))
        log.debug(
            f"Replaced parameter {name} with remote tensor (local_storage_id={local_storage_id}, remote_storage_id={remote_storage_id})"
        )
    ...
    for name, buffer_meta in buffer_metadata.items():
        ...
        # Get the remote storage ID that contains the actual buffer data
        remote_storage_id = buffer_meta["storage_id"]
        ...
        log.debug(
            f"Replaced buffer {name} with remote tensor (local_storage_id={local_storage_id}, remote_storage_id={remote_storage_id})"
        )
    ...
    _handle_tied_weights(model, state_dict_metadata, storage_mapping)
    ...
    # Collect parameters
    for name, param_metadata in state_dict_metadata.items():
        # Find the corresponding local storage ID for this parameter name
        for local_storage_id, remote_storage_id in storage_mapping.items():
            if param_metadata["storage_id"] == remote_storage_id:
    ...
    for name, buffer_meta in buffer_metadata.items():
        for local_storage_id, remote_storage_id in storage_mapping.items():
            if buffer_meta["storage_id"] == remote_storage_id:
                local_storage_ids.append(local_storage_id)
                parameter_names.append(name)
                break
    ...
    log.info(
        f"✅ Successfully created {model_type} model with {len(created_storage_ids)} remote tensors"
    )
    return model
