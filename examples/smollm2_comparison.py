...
def load_model_local(machine):
    ...
    return AutoModelForCausalLM.from_pretrained("HuggingFaceTB/SmolLM2-135M-Instruct").to(machine.device())


def main():
    parser = argparse.ArgumentParser(description="SmolLM2 with manual token generation")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--remote", action="store_true", help="Load model directly on remote GPU")
    group.add_argument("--local", action="store_true", help="Load model locally then transfer to remote")
